...
interface HeaderProps {
  ...
  // E-commerce Manager
  setShowEcommerceManager?: (show: boolean) => void
  isEcommerceInstalled?: boolean
...
export const Header: React.FC<HeaderProps> = ({
  ...
  isBookingInstalled,
  setShowEcommerceManager,
  isEcommerceInstalled,
    ...
    <header className={`${theme.headerBg} border-b ${theme.headerBorder} flex items-center h-9 shadow-sm`}>
      ...
      <div className="flex items-center h-full">
        ...
        <div className={`flex items-center h-full border-l ${theme.inputBorder} pl-2 ml-2`}>
          ...
          {/* UAS Manager Button - Only visible when UAS feature is installed */}
          {isUasInstalled && setShowUasManager && (
            <button
              onClick={() => setShowUasManager(true)}
              className={`p-1.5 rounded transition ml-1 ${theme.headerText} ${theme.headerHover}`}
              title="User Access System Manager"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {/* E-commerce Manager Button - Only visible when E-commerce feature is installed */}
          {isEcommerceInstalled && setShowEcommerceManager && (
            <button
              onClick={() => setShowEcommerceManager(true)}
              className={`p-1.5 rounded transition ml-1 ${theme.headerText} ${theme.headerHover}`}
